...
\begin{proof}
  ...
  \begin{equation}
    L(\a, \lambda) = \frac{1}{2} \| J \a + r \|^2 + \frac{\lambda}{2} \left( \left\| D \a \right\| - \Delta^2 \right),
  \end{equation}
  ...
  Assuming that strong duality holds, the dual problem $\sup_{\lambda \geq 0} \inf_{\a} L(\a, \lambda)$ has the same optimal value. The inner problem can be re-written as
  \begin{equation}
    \inf_{\a} L(\a, \lambda) = \frac{1}{2} \a^T ( J^T J + \lambda D^T D ) \a + r^T J \a - \lambda \frac{\Delta^2}{2}.
  \end{equation}
  This inner problem has value $-\infty$ unless $J^T J + \lambda D^T D \succeq 0$, so the outer supremum restricts $\lambda$ to values that imply positive semi-that imply positive semi-definiteness. In this case the finite optimal value is attained for $\left(J^T J + \lambda D^T D \right) \a = -J^T r$ which reduces the dual problem to
  \begin{equation}
...
\begin{algorithm}
  ...
  $p = \frac{ \| r \|^2 - \| f(\x \oplus_r \a_\mathrm{LM}) - \y \|^2 }{\| r \|^2 - \| r + J \a_\mathrm{LM} \|^2}$ \tcp*[r]{actual to predicted reduction ratio}
  \lIf(\tcp*[f]{decrease trust region}){$p < 0.25$}{ $\Delta^{k+1} = \Delta^k / 2$}
  ...
  \lIf(\tcp*[f]{reject step}){$p \leq 0.0001$}{$\x^{k+1} = \x^k$}
...
Calculation of the actual to predicted reduction ratio can with be rewritten as
\begin{equation}
