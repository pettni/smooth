...
auto dr_autodiff(_F && f, _Wrt && x)
{
  ...
  auto nx = std::apply([]<typename... Args>(Args && ... args) { return (args.size() + ...); }, x);
  ...
  typename decltype(val.template cast<AdScalar>())::PlainObject val_ad = val.template cast<AdScalar>();
